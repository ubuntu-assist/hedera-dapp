...
async function contractExecuteFcn(walletData, accountId, tokenId, contractId) {
  ...
  const transferTx = await new TransferTransaction()
    ...
    .addHbarTransfer(contractId, new Hbar(1))
  ...
  const transferTxRx = await provider.getTransactionReceipt(
    transferTxSubmit.transactionId
  )
  console.log(
    `- HBAR transfer from Operator to contract: ${transferTxRx.status.toString()}`
  )

  const bCheck = await signer.getAccountBalance()
  console.log(
    `- Operator balance: ${bCheck.tokens._map.get(
      tokenId.toString()
